...
for i=0:length(z)-1
    pupil(aperture)=exp(kz_of_kxky(aperture)*(.5*1i*klen/sxy*z(i))).*sqrt(cosTheta(aperture))^2;
    fpropmat(:,:,i)=pupil;
end
clear pupil
%fpropmat
a=abs(ft(squeeze(fpropmat(:,:,1)))).^2;b=squeeze(psf(:,:,6-1));
pp4(:,:,0)=a./max(a);
pp4(:,:,1)=b./max(b);
%
hold off
...
mma_overlay=mma+repmat(~aperture,[1 1 nline nline])
